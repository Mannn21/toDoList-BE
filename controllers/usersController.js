        ...
        if(!search) {
            try {
                if(password !== confPassword) {
                    return response(400, "Password not compare", null, "Bad request", res)
                }
                if(password === confPassword) {
                    const encryptedPassword = await bcrypt.hash(password, 10)
                    if(!encryptedPassword) {
                        return response(500, "Generate Password error", null, "Internal server error", res)
                    }
                    if(encryptedPassword) {
    ...
    try {
        ...
        if(search) {
            try {
                ...
                if(compare) {
                    const deleted = await prisma.user.delete({where: {email: search.email}})
...
const login = async (req, res) => {
    try {
        ...
        if(search) {
            ...
            if(compare) {
                try {
                    const searchLogin = await prisma.login.findUnique({where: {user_id: search.id}})
                    const name = search.name
                    const email = search.email
                    const user_id = search.id
                    const accessToken = jwt.sign({name, email, user_id}, process.env.ACCESS_TOKEN_SECRET, {
                        expiresIn: '20s'
                    })
                    const refreshToken = jwt.sign({name, email, user_id}, process.env.REFRESH_TOKEN_SECRET, {
                        expiresIn: '1d'
                    })
                ...
                try {
                    ...
                    if(user) {
                        jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, decode) => {
                            ...
                            return response(200, "Refresh Token Success", accessToken, "Ok", res)
                        })
                    }
                } catch(err) {
                    return response(500, err.message, null, "Internal server error", res)
            ...
            if(search) {
                try {
                    ...
                    req.session.destroy(err => {
                        if(err) return response(400, "Logout failed", null, "Bad request", res)
